...
export function TaskDetailsTextEditor({ saveUpdate }) {
    ...
    return (
        <section className="TaskDetailsTextEditor">
            ...
            <label htmlFor="color-input" >
                <button className='icon-btn paint-roller clickable clear icon-btn size-32'></button>
            </label>
            <input style={{ display: 'none' }} id='color-input' type="color" onInput={event => editor.chain().focus().setColor(event.target.value).run()}
                value={editor.getAttributes('textStyle').color} data-testid="setColor"/>
